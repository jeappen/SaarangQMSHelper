...
import org.saarang.qmshelper.RegisteredUser.ExistingDetails;
				...
				final boolean a = activeNetworkInfo != null && activeNetworkInfo.isConnected();
					...
					if (a) {
						if(i==-1 || sid.length()-i!=sizeofuserid+2 ) //check if shaastra id is 5 digits
							Toast.makeText(getApplicationContext(), "Please enter a valid "+getString(R.string.fest_name)+"ID", Toast.LENGTH_LONG).show();
						else{	
