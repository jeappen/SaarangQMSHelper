...
import org.saarang.qmshelper.reused.Database;
import org.saarang.qmshelper.reused.Display;
import org.saarang.qmshelper.reused.EventDatabase;
import org.saarang.qmshelper.reused.Register;
...
//						break;
    ...
    class getEvents extends AsyncTask<String, String, String> {
        ...
        JSONParser jsonParser = new JSONParser();
        int error;
        String errmsg,user_id,separator=getString(R.string.event_separator);
        @Override
        ...
        protected String doInBackground(String... params) {
            ...
            String url = "api/mobile/display_events/";
            JSONObject json = jsonParser.makeHttpRequest(url, "GET", paramse,null );//app.getadmintoken()
            ...
            try{
                ...
                {	String team="-1";
                    ...
                    if(!event.getString("name").equals("Startup Hive"))
                        add=separator+event.getString("name")+separator+event.getString("id")+separator+team;
                ...
                for(int k=0;k<extra_depts.length;k++){
                    String add="";
                    add=separator+extra_depts[k]+separator+String.valueOf(++last_id)+separator+"0";
                    result=result+add;
                }
        ...
        protected void onPostExecute(String file_url) {
            ...
            if (error==0) {
                List<String> myStrings =Arrays.asList(result.split(separator));
                //myStrings.remove(0);
                db.open();
                db.update();
